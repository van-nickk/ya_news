...
class TestCommentEditDelete(TestCase):
    ...
    NEW_COMMENT_TEXT = 'Updated comm'

    @classmethod
    def setUpTestData(cls):
        cls.news = News.objects.create(title='Header', text='Text')
        news_url = reverse('news:detail', args=(cls.news.id,))
        cls.url_to_comments = news_url + '#comments'
        cls.author = User.objects.create(username='Comm author')
        cls.author_client = Client()
        cls.author_client.force_login(cls.author)
        cls.reader = User.objects.create(username='Comm reader')
        ...
        cls.edit_url = reverse('news:edit', args=(cls.news.id,))
        cls.delete_url = reverse('news:delete', args=(cls.news.id,))
